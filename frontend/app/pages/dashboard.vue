  ...
  <div>
    ...
    <div class="grid gap-3 sm:grid-cols-3">
      <div class="rounded-xl border border-white/[0.04] bg-white/[0.02] p-5">
        <div class="text-[11px] font-medium uppercase tracking-widest text-white/20">Balance</div>
        <div class="mt-2 font-display text-3xl font-extrabold tabular-nums text-red-400">{{ user?.balance ?? 0 }}</div>
        <div class="mt-1 text-[11px] text-white/15">Toll Points</div>
      </div>
      <div class="rounded-xl border border-white/[0.04] bg-white/[0.02] p-5">
        <div class="text-[11px] font-medium uppercase tracking-widest text-white/20">Characters</div>
        <div class="mt-2 font-display text-3xl font-extrabold tabular-nums text-white">{{ allPlayers.length }}</div>
        <div class="mt-1 text-[11px] text-white/15">Total on account</div>
      </div>
      <div class="rounded-xl border border-white/[0.04] bg-white/[0.02] p-5">
        <div class="text-[11px] font-medium uppercase tracking-widest text-white/20">Membership</div>
        <div class="mt-2 font-display text-lg font-bold text-white/40">Free</div>
        <div class="mt-1 text-[11px] text-white/15">No active subscription</div>
      </div>
    </div>
    ...
    <div class="mt-10">
      <div class="mb-4 flex items-center justify-between">
        <h2 class="font-display text-lg font-bold uppercase tracking-wider">Characters</h2>
      ...
      <div v-else-if="allPlayers.length === 0" class="rounded-xl border border-white/[0.04] bg-white/[0.02] py-16 text-center">
        ...
        <p class="text-[14px] font-medium text-white/30">No characters yet</p>
        <p class="mt-1 text-[12px] text-white/15">Log into the game to create your first character</p>
      </div>
    ...
    <div class="mt-10 grid gap-3 sm:grid-cols-3">
      <NuxtLink to="/shop"
        ...
        <div>
          <div class="text-[14px] font-semibold transition-colors group-hover:text-red-400">Toll Shop</div>
          <div class="mt-0.5 text-[12px] text-white/20">Cosmetics, mounts & more</div>
        </div>
      </NuxtLink>
      <button @click="showPromoModal = true"
        ...
        <div>
          <div class="text-[14px] font-semibold transition-colors group-hover:text-gold-400">Promo Code</div>
          <div class="mt-0.5 text-[12px] text-white/20">Activate a code for rewards</div>
        </div>
      </button>
      <NuxtLink to="/profile"
        class="group flex items-center gap-4 rounded-xl border border-white/[0.04] bg-white/[0.02] p-5 transition-all duration-300 hover:border-white/[0.08] hover:bg-white/[0.03]">
        ...
        <div>
          <div class="text-[14px] font-semibold transition-colors group-hover:text-white/80">Account settings</div>
          <div class="mt-0.5 text-[12px] text-white/20">Email, password, security</div>
        </div>
      </NuxtLink>
    </div>
    ...
    <div class="mt-10 rounded-xl border border-white/[0.04] bg-white/[0.02] px-6 py-5">
      <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="text-[14px] font-semibold text-white/60">Have questions or need help?</div>
            <div class="mt-0.5 text-[12px] text-white/20">Our support team is ready to assist you</div>
          </div>
        </div>
        <NuxtLink to="/tickets"
    ...
    <AppModal :open="showPromoModal" title="Promo Code" @close="closePromoModal">
      <template #icon>
        <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gold-500/10">
      ...
      <form @submit.prevent="handleActivatePromo" class="space-y-4">
        <div>
          <label class="mb-1.5 block text-[12px] font-medium text-white/40">Enter your code</label>
          <input
        ...
        <div class="flex gap-3">
          <AppButton variant="secondary" @click="closePromoModal">Cancel</AppButton>
