<template>
  <div>
    ...
    <h1 class="font-display text-3xl font-extrabold uppercase tracking-tight">Reset password</h1>
    <p class="mt-2 max-w-xs text-[14px] text-white/30">Enter your email and we'll send you a link to reset your password.</p>
    ...
    <div v-if="sent" class="mt-10 rounded-lg border border-emerald-500/15 bg-emerald-500/[0.05] px-5 py-4">
      <div class="flex items-start gap-3">
        ...
        <div>
          <p class="text-[14px] font-medium text-emerald-300">Check your email</p>
          <p class="mt-1 text-[13px] text-emerald-300/50">If an account with that email exists, we sent a password reset link. Check spam folder if you don't see it.</p>
        </div>
      </div>
    </div>
    ...
    <form v-else class="mt-10 space-y-5" @submit.prevent="handleSubmit">
      ...
      <AppButton type="submit" :loading="isLoading" loading-text="Sending..." block>Send reset link</AppButton>
    </form>
  </div>
</template>
