<template>
  <div>
    <div class="mb-8">
      <h1 class="font-display text-2xl font-extrabold uppercase tracking-tight lg:text-3xl">{{ $t('characters.title') }}</h1>
      <p class="mt-1 text-[13px] text-white/25">{{ $t('characters.subtitle') }}</p>
    </div>
    ...
    <EmptyState v-else-if="allPlayers.length === 0" :title="$t('dashboard.noCharactersYet')" :subtitle="$t('dashboard.createFirstCharacter')">
      <template #icon>
...
async function handleTeleport(player: Player) {
  ...
  try {
    ...
    teleportResult[player.id] = { success: true, message: res.message || t('characters.teleportSuccess') }
  } catch (e: unknown) {
    const err = e as { data?: { message?: string } }
    teleportResult[player.id] = { success: false, message: err.data?.message || t('characters.teleportFailed') }
  } finally {
