    ...
    <section class="rounded-xl border border-white/[0.04] bg-white/[0.02]">
      ...
      <div class="px-6 py-5">
        ...
        <template v-else-if="tfa.enabled">
          <div class="space-y-4">
            ...
            <div class="rounded-lg border border-white/[0.04] bg-white/[0.02] px-4 py-3">
              <p class="text-[12px] leading-relaxed text-white/25">{{ $t('security.recoveryCodes') }}</p>
            </div>
            <AppButton variant="secondary" @click="tfa.enabled = false; tfa.method = ''">{{ $t('security.disable2fa') }}</AppButton>
          </div>
        </template>
        ...
        <div v-else class="space-y-4">
          ...
          <p class="text-[13px] leading-relaxed text-white/30">{{ $t('security.setupLater') }}</p>
        </div>
      </div>
    </section>
  </div>
