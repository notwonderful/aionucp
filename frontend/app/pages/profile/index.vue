  ...
  <section class="rounded-xl border border-white/[0.04] bg-white/[0.02]">
    <div class="border-b border-white/[0.04] px-6 py-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="font-display text-[15px] font-bold uppercase tracking-wider">Account</h2>
          <p class="mt-0.5 text-[11px] text-white/20">Manage your profile information</p>
        </div>
      </div>
    </div>
    ...
    <form class="max-w-lg space-y-4 px-6 py-5" @submit.prevent="handleUpdateProfile">
      <div>
        <label class="mb-1.5 block text-[12px] font-medium text-white/40">Username</label>
        <div class="flex items-center gap-3 rounded-lg border border-white/[0.04] bg-white/[0.015] px-4 py-3">
          ...
          <span class="ml-auto rounded bg-white/[0.04] px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-white/15">read only</span>
        </div>
      </div>

      <FormInput v-model="form.email" id="profile-email" label="Email" type="email" required autocomplete="email" :error="errors.email?.[0]" />
      <AlertMessage :message="success" variant="success" />
      <AppButton type="submit" :loading="loading" loading-text="Saving...">Save changes</AppButton>
    </form>
  </section>
</template>
